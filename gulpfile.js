...
gulp.task('generate-config', () => {
  return new Promise((resolve, reject) => {
    ...
    if ('SHOWTUNES_SPOTIFY_CLIENT_ID' in process.env) {
      configJson.auth.spotify.clientId = process.env.SHOWTUNES_SPOTIFY_CLIENT_ID;
    }
    if ('SHOWTUNES_SPOTIFY_CLIENT_SECRET' in process.env) {
      configJson.auth.spotify.clientSecret = process.env.SHOWTUNES_SPOTIFY_CLIENT_SECRET;
    }
    if ('SHOWTUNES_SPOTIFY_SCOPES' in process.env) {
      configJson.auth.spotify.scopes = process.env.SHOWTUNES_SPOTIFY_SCOPES;
    }
    if ('SHOWTUNES_SPOTIFY_TOKEN_URL' in process.env) {
      configJson.auth.spotify.tokenUrl = process.env.SHOWTUNES_SPOTIFY_TOKEN_URL;
    }
